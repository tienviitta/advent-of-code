...
export default function solve(input) {
  ...
  let results = 0;
  banks.forEach((bank) => {
    const nums = Array.from(bank).map(Number);
    console.table(nums);
    const tmp = mapNextGreater(nums);
    console.table(tmp);
  });
  return results;
}
...
/**
 * Finds the next greater element for each element mapping
 * @param nums - Array of numbers to find next greater elements for
 * @returns Array where each element is the next greater element for corresponding element in nums1
 */
function mapNextGreater(nums) {
  ...
  for (const currentNumber of nums.reverse()) {
